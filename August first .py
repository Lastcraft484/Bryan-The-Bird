...
@bot.command()
async def test(ctx):
    await ctx.send_file('brozart.jpg')
...
async def info(ctx):
    ...
    embed.add_field(name="Author", value="Cat")
...
async def help(ctx):
    ...
    embed.add_field(name="?coords", value="Submit a coord for a location(format xCoord yCoord zCoord nameCoord", inline=False)
    embed.add_field(name="?reqcoord", value="Request a coord for a location (example: ?reqcoord (name of coord)", inline=False)
